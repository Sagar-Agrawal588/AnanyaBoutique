...
const ExclusivePreviewCard = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -inset-[1px] rounded-3xl bg-gradient-to-r ${resolvedTheme.accent} opacity-90`}
      />
      <div
        className={`premium-glow absolute -inset-3 rounded-[30px] bg-gradient-to-r ${resolvedTheme.badgeRich} opacity-35 blur-2xl transition-opacity duration-500 group-hover:opacity-90`}
      />

      <div
        className={`relative overflow-hidden rounded-[22px] border ${resolvedTheme.border} bg-white/85 backdrop-blur-xl`}
      >
        ...
        >
          <div className="relative aspect-[4/3] overflow-hidden rounded-t-[22px] bg-slate-50">
            ...
            <div className="absolute left-3 top-3">
              <PremiumBadge className={resolvedTheme.badgeRich} />
            </div>
          </div>
          ...
          <div className="px-4 pb-4 pt-3">
            ...
            <div className="mt-3 flex items-end justify-between">
              ...
              {!isLocked ? (
                <span
                  className={`inline-flex h-9 w-9 items-center justify-center rounded-full border ${resolvedTheme.softBorder} ${resolvedTheme.softBg} ${resolvedTheme.softText} shadow-sm`}
                >
