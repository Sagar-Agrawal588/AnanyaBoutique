...
const Checkout = () => {
  ...
  const displayTotal = Math.max(Math.round(Number(total || 0)), 0);
  const displaySubtotal = round2(
    displayTotal / (1 + CHECKOUT_GST_RATE_PERCENT / 100),
  );
  const displayTax = round2(displayTotal - displaySubtotal);
  const displayDiscountTotal = round2(couponDiscount + referralDiscountDisplay);
  const displaySummarySubtotal = round2(displaySubtotal + displayDiscountTotal);
  ...
  const buildCheckoutItemsHash = (sourceItems = []) => {
    const normalized = (sourceItems || [])
      ...
      .sort((a, b) => {
        const keyA = `${a.type}:${a.id}:${a.variantId || ""}`;
        const keyB = `${b.type}:${b.id}:${b.variantId || ""}`;
  ...
  const getCheckoutItemsForSubmit = useCallback(async () => {
    ...
    const fallbackHash = buildCheckoutItemsHash(fallbackItems);
    if (!authToken || typeof fetchCart !== "function") {
      return {
        items: fallbackItems,
        changed: false,
      };
    ...
    try {
      ...
      if (Array.isArray(refreshed?.items)) {
        const refreshedHash = buildCheckoutItemsHash(refreshed.items);
        return {
          items: refreshed.items,
          changed: refreshedHash !== fallbackHash,
        };
      }
    } catch {
    ...
    try {
      ...
      if (runtimePreviewPricing) {
        const runtimeTotal = round2(
          Number(runtimePreviewPricing.finalAmount || 0),
        );
        if (runtimeTotal > 0 && Math.abs(runtimeTotal - total) > 0.01) {
          setSnackbar({
            open: true,
              ...
              <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 md:p-8 border border-white/50 shadow-xl shadow-gray-200/50">
                ...
                <div className="space-y-4">
                  {cartItems.map((item, index) => {
                    ...
                    return (
                      <div
                        key={data.cartKey || data.id || index}
