...
export default function PolicyPage() {
  ...
  const themeKey = policy?.theme?.style || DEFAULT_THEME.style;
  const layout = policy?.theme?.layout || DEFAULT_THEME.layout;
  const theme = THEME_PRESETS[themeKey] || THEME_PRESETS.mint;
  const sanitizedContent = buildSafeHtml(policy?.content);
  const effectiveDateLabel = policy?.effectiveDate
