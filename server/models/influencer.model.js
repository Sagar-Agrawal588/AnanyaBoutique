...
const normalizeBaseUrl = (value) => {
  const normalized = getFirstConfiguredUrl(value).replace(/^['"]|['"]$/g, "");
...
const resolveInfluencerClientBaseUrl = () => {
  const candidates = [
  ...
  return candidates[0] || DEFAULT_PUBLIC_CLIENT_URL;
};

const buildInfluencerPublicUrl = (pathname, searchParams = {}) => {
  const baseUrl = resolveInfluencerClientBaseUrl();
...
const influencerSchema = new mongoose.Schema(
  {
    ...
    // Secure password hash for collaborator portal sign-in
    portalPasswordHash: {
      type: String,
      default: "",
      select: false,
...
influencerSchema.virtual("referralUrl").get(function () {
  return buildInfluencerPublicUrl("/", { ref: this.code });
});

influencerSchema.virtual("portalLoginUrl").get(function () {
  return buildInfluencerPublicUrl("/affiliate/login", { code: this.code });
});
