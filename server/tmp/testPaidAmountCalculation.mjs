...
async function testCalculation() {
  try {
    ...
    console.log(
      "Data from export (showing rows where paid amount was calculated):\n",
    );
    ...
    const totalDiscountIdx =
      headers.findIndex(
        (h) => String(h || "").trim() === "Total Discount (Rs)",
      ) + 1;
    const paidIdx =
      headers.findIndex((h) =>
        String(h || "").includes("Customer Paid (After Discount)"),
      ) + 1;
    const productIdx =
      headers.findIndex((h) => String(h || "").includes("Product Name")) + 1;
    const customerIdx =
      headers.findIndex((h) => String(h || "").includes("Customer")) + 1;

    console.log(
      `Relevant columns: Price[${priceIdx}], Total Discount[${totalDiscountIdx}], Paid[${paidIdx}], Product[${productIdx}], Customer[${customerIdx}]\n`,
    );
    ...
    for (let i = 2; i <= Math.min(ws.rowCount, 50); i++) {
      ...
      if (paidAmount === 0 && price > 0) {
        ...
        console.log(
          `   Price: ${price}, Total Discount: ${totalDiscount}, Paid: ${paidAmount}`,
        );
        console.log();
      }
    }
    ...
    if (zeroCount === 0) {
      console.log(
        `✅ NO ZERO VALUES FOUND - All calculations working correctly!`,
      );
    } else {
      console.log(
        `⚠️  Found ${zeroCount} rows with zero paid amounts that shouldn't be zero`,
      );
    }
  } catch (error) {
