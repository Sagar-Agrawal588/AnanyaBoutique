...
async function main() {
  ...
  if (templates.length === 0) {
    console.error("No approved templates available. Aborting.");
    await mongoose.disconnect();
    process.exit(2);
  }

  const template =
    templates.find((t) => t.status === "approved") || templates[0];
  console.log("Using template:", template.name, "language:", template.language);
